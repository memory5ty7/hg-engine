...
BOOL LONG_CALL MonDiesFromResidualDamage(struct BattleStruct *ctx, u32 attacker, u32 attackerCondition, BOOL isSeeded)
{
    ...
    } else if (ctx->field_condition & FIELD_CONDITION_HAIL_ALL || ctx->field_condition & FIELD_CONDITION_SNOW_ALL) {
...
BOOL LONG_CALL IsMonInflictedWithAnyNegativeStatus(struct BattleStruct *ctx, u32 attacker)
{
    ...
    if (ctx->battlemon[attacker].effect_of_moves & (MOVE_EFFECT_FLAG_YAWN | MOVE_EFFECT_FLAG_LEECH_SEED_RECIPIENT | MOVE_EFFECT_FLAG_PERISH_SONG)) {
        return TRUE;
    }
